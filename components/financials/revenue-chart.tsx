...
Sparklend: Protocol treasury's share of borrower interest across SparkLend markets, determined by each asset's Reserve Factor. Combines net interest, flashloan premiums, and liquidation bonuses since these are all SparkLend-channel revenue. Toggle Flashloan and Liquidation legend entries to separate them out.
...
Distribution Rewards: Rate-based accrual paid by Sky to Spark for growing USDS/sUSDS demand (base ~20 bps/yr on tagged USDS balance, up to +30 bps boost). Settled once per Monthly Settlement Cycle as an on-chain USDS mint from MCD Pause Proxy (0xbE28...38f3) to Spark Proxy (0x3300...f8c4).
...
Known reconciliation vs Blockworks (in-flight):
  • Sparklend runs high because our Reserve Factor calc includes USDS/DAI reserves; Blockworks excludes them.
  • Distribution Rewards runs ~2x high because our current amortization double-counts the trailing daily rate on top of the actual settlement mint. A rate-accrual rewrite is queued as a follow-up.
  • SLL differs because Blockworks pulls ALM Proxy yield on-chain; we use DefiLlama's aggregate.

We show our own numbers here — the transparent gap and its explanation is the honest posture. The queued fixes will collapse most of the difference.`
...
export function RevenueChart({ daily }: RevenueChartProps) {
  ...
  return (
    <ChartFrame
      ...
      footnote={
        <span>
          Series match Blockworks Research's Spark Total Revenue chart. Numbers may not (yet) match exactly:
          our Sparklend calc still includes USDS/DAI reserves that Blockworks strips; our Distribution
          Rewards series over-counts vs their rate-accrual model. Reconciliation queued as follow-up.
          The last bar covers the current partial period — tooltip labels it "partial".
        </span>
      }
    >
